export function AccountCard({
  ...
  return (
    <div className="bg-[rgba(255,255,255,0.04)] backdrop-blur border border-white/10 rounded-2xl p-4 sm:p-6 flex flex-col sm:flex-row items-start gap-4 sm:gap-5">
      {profile.avatar ? (
        <img
        ...
        <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-white/10 flex items-center justify-center text-2xl">
          {profile.displayName?.charAt(0) || "?"}
        </div>
      )}
      <div className="flex-1 min-w-0 w-full">
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          <h2 className="font-bold text-base sm:text-lg truncate">{profile.displayName}</h2>
